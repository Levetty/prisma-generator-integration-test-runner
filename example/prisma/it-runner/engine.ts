...
import { PrismaClient, Prisma, Link, User } from ".prisma/client";

export const modelNames = ["Link", "User"];
...
export async function applyRecordSet(
	...
	await Promise.all([
		recordSet.Link
			? db.$executeRawUnsafe(`DELETE FROM "Link";`)
			: Promise.resolve(),
		recordSet.User
	...
	await Promise.all([
		recordSet.Link
			? db.link.createMany({ data: recordSet.Link ?? [] })
			: Promise.resolve(),
		recordSet.User
			? db.user.createMany({ data: recordSet.User ?? [] })
			: Promise.resolve(),
	]);

	const autoIncrementResetJobs: Promise<number>[] = [];
	if (autoIncrementReset.Link)
		autoIncrementResetJobs.push(
			db.$executeRawUnsafe(
				`SELECT SETVAL ('Link_id_seq', ${autoIncrementReset.Link}, false);`,
			),
		);
	if (autoIncrementReset.User)
		autoIncrementResetJobs.push(
			db.$executeRawUnsafe(
				`SELECT SETVAL ('User_id_seq', ${autoIncrementReset.User}, false);`,
			),
		);
...
export async function importRecordSet(db: PrismaClient): Promise<RecordSet> {
...
export async function assertRecordSet(
	...
	if (recordSet.Link)
		assertFunc(recordSet.Link, await db.link.findMany(), assertSortKey?.Link);
	if (recordSet.User)
		assertFunc(recordSet.User, await db.user.findMany(), assertSortKey?.User);
}
