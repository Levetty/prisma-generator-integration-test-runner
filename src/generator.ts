...
function renderTemplate(template: string, variables: { [name: string]: string }): string {
  let out = template;
...
export async function generate(basePath: string, options: GeneratorOptions) {
  ...
  const template = (await fs.promises.readFile(path.join(__dirname, "template", "engine.ts.template"))).toString();
            ...
            ? [
              {
                ...
                fromFields: field.relationFromFields ? field.relationFromFields : [],
    ...
    sortedModels
      .map((models) =>
        ...
        [`await Promise.all([`]
          .concat(
            models.map((model) => {
              const jsonFields = model.fields.filter((f) => f.fieldType === "Json");
              let mapString = "";
              if (jsonFields.length > 0) {
                const jsonFieldsString = jsonFields
                  .map(
                    (jf) =>
                      `${jf.fieldName}: ${jf.isRequired ? "" : `r.${jf.fieldName} === undefined ? undefined : `}r.${
                        jf.fieldName
                      } === null ? Prisma.JsonNull : r.${jf.fieldName}`
                  )
                  .join(", ");
                mapString = `.map(r => ({ ...r, ${jsonFieldsString}}))`;
              }
              return `  recordSet.${model.dbName} ? db.${toLower(model.name)}.createMany({ data: recordSet.${
                model.dbName
  ...
  const assertRecordSetAssertions = formatLines(
    models.map((model) => {
      return `if (recordSet.${model.dbName}) assertFunc(recordSet.${model.dbName}, await db.${toLower(
        model.name
      )}.findMany(), assertSortKey?.${model.dbName});`;
    }),
    2
  );
  ...
  const runner = (await fs.promises.readFile(path.join(__dirname, "template", "runner.ts"))).toString();
