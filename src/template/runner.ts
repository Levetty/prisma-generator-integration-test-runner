...
function assertRecords<Model>(expected: Model[], actual: Model[], sortKeys?: (keyof Model)[]) {
...
export function _TEST_runIntegrationTest<Return>(db: PrismaClient, t: TestCase<Return> | (() => TestCase<Return>)): jest.ProvidesCallback {
  return async function () {
    try {
      ...
      const ret = await tt.method(db).then((value) => (tt.returnsPrehook ? tt.returnsPrehook(value) : value));
      ...
      if (tt.mutates) await assertRecordSet(db, tt.mutates, assertRecords, tt.mutatesSortKey);
    } finally {
      await db.$disconnect();
    }
  };
